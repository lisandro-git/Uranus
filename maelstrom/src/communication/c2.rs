...
pub struct Cohort {
    /// Cohort's ID
    pub c2_id: Vec<u8>,
    /// Wether or not the cohort is a leader
    pub is_hq: bool,
    /// A vectored list of the peers in the cohort
...
pub struct Device_stream {
    /// Ip address of the device (The device being a C2 server)
    pub ip_address: String,
    /// Wether or not the device is authenticated
    pub authenticated: bool,
    /// Wether or not the device is connected
    pub connected: bool,
    /// Device's encryption key used for encrypting and decrypting messages
...
#[derive(Debug, Clone, Serialize, Deserialize, DeepSizeOf, Hash)]
pub struct Bot {
    /// Bot's UID generated by the bot himself
    #[serde(with = "serde_bytes")]
    pub uid: Vec<u8>,
    /// Bot's version
    #[serde(with = "serde_bytes")]
    pub version: Vec<u8>,
    /// Command and command's result data
